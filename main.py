...
def _hellinger_distance_py(P, Q):
	...
	for i in range(len(P[0])):
		...
		for j in range(len(P)):
			d = math.sqrt(P[j][i]) - math.sqrt(Q[j][i])
			h += d**2
...
# Referenzimplementierung 
def _select_best_cols_py(P, Q, H):
	"""
...
from implementation import select_best_cols

## Test 1
H = _hellinger_distance_py(data_1_P, data_1_Q)
diffs = _select_best_cols_py(data_1_P, data_1_Q, H) - select_best_cols(data_1_P, data_1_Q, H)
if np.abs(diffs).sum() > 1e-05:
...
for i in range(3):
	...
	result = select_best_cols(P, Q, H)
	correct = _select_best_cols_py(P, Q, H)
...
# Beispiel erzeugen
#	Test 1 
Q = random_distribution_matrix(seed=3247, shape=(100, 8))
P = random_distribution_matrix(seed=2234, shape=(100, 8))
H = _hellinger_distance_py(P, Q)
